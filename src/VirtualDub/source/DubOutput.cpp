...
IVDMediaOutput *VDAVIOutputRawSystem::CreateSegment() {
	vdautoptr<AVIOutputRaw> pOutput(new AVIOutputRaw);
...
void VDAVIOutputRawSystem::CloseSegment(IVDMediaOutput *pSegment, bool bLast) {
	vdautoptr<AVIOutputRaw> pFile(static_cast<AVIOutputRaw *>(pSegment));
...
bool VDAVIOutputRawSystem::AcceptsVideo() {
	return false;
}
...
void VDAVIOutputFilmstripSystem::CloseSegment(IVDMediaOutput *pSegment, bool bLast) {
	vdautoptr<AVIOutputFLM> pFile(static_cast<AVIOutputFLM *>(pSegment));
	pFile->finalize();
}

void VDAVIOutputFilmstripSystem::SetVideo(const AVIStreamHeader_fixed& asi, const void *pFormat, int cbFormat) {
	mVideoStreamInfo = asi;
	mVideoFormat.resize(cbFormat);
	memcpy(&mVideoFormat[0], pFormat, cbFormat);
}
...
void VDAVIOutputSegmentedSystem::CloseSegment(IVDMediaOutput *pSegment, bool bLast) {
