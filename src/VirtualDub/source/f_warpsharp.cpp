...
void VDVFilterWarpSharp::Start() {
	...
	mBumpPitch = (pxsrc.w+7)&~7;

	if (!(mpBumpBuf = new_nothrow unsigned char[mBumpPitch * pxsrc.h]))
		ff->ExceptOutOfMemory();

	memset(mpBumpBuf, 0, mBumpPitch*pxsrc.h);

	mDisplacementRowMap.resize(pxsrc.w*2, 0);

	vbmBump.init(mpBumpBuf, mBumpPitch, pxsrc.h, 8);

	if (!(veffBlur = VCreateEffectBlurHi(&vbmBump)))
		ff->ExceptOutOfMemory();
...
xloop:
	...
	static void InterpolateBicubicRowDisplacement32(uint32 *dst, const uint32 *src, ptrdiff_t pitch, const int *disprow, uint32 w, const int *cubicTab) {
		for(uint32 i=0; i<w; ++i) {
			...
			const Pixel32 *src0 = (const uint32 *)((const char *)src + pitch*((dispy>>8)-1)) + (dispx>>8);
			++src;
	...
	static void InterpolateBicubicRowDisplacement32MMX(uint32 *dst, const uint32 *src, ptrdiff_t pitch, const int *disprow, uint32 w, const int *cubicTabMMX) {
		...
		for(uint32 i=0; i<w; ++i) {
			...
			const Pixel32 *src2 = (const uint32 *)((const char *)src + pitch*((dispy>>8)-1)) + (dispx>>8);
...
void VDVFilterWarpSharp::Run() {
	...
	if (pxdst.format == nsVDXPixmap::kPixFormat_XRGB8888)
		...
		if (pxdst.format != nsVDXPixmap::kPixFormat_Y8) {
...
extern const VDXFilterDefinition filterDef_warpsharp = VDXVideoFilterDefinition<VDVFilterWarpSharp>(NULL, "warp sharp", "Tightens edges in an image by warping the image toward edge boundaries. (Version 1.2)\n\n[Assembly optimized][Requires MMX]");
