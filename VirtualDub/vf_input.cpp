...
sint32 VDVideoFilterInput::Run() {
	const void *p = mpSource->getFrame(mpContext->mpOutput->mFrameNum);

	mpContext->AllocFrame();

	VDPixmapBlt(*mpContext->mpDstFrame->mpPixmap, VDAsPixmap(VBitmap((void *)p, mpSource->getDecompressedFormat())));
