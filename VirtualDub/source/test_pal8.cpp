...
void VDCreateTestPal8Video(VDGUIHandle h) {
	...
	try {
		...
		VideoSource *pVS = pIF->videoSrc;
		const uint32 frames = pVS->getLength();
		...
		hdr.wPriority	= 0;
		hdr.wLanguage	= 0;
		hdr.dwScale		= pVS->getStreamInfo().dwScale;
		hdr.dwRate		= pVS->getStreamInfo().dwRate;
		hdr.dwStart		= 0;
		hdr.dwLength	= 0;
		...
		hdr.rcFrame.left = 0;
		hdr.rcFrame.top = 0;
		hdr.rcFrame.right = pxsrc.w;
		hdr.rcFrame.bottom = pxsrc.h;
		...
		for(int i=0; i<252; ++i) {
			pal[i].rgbRed = (i/42)*51;
			pal[i].rgbGreen = (((i/6)%7)*85)>>1;
			pal[i].rgbBlue = (i%6)*51;
			pal[i].rgbReserved = 0;
		}
		...
		ProgressDialog dlg((HWND)h, "Processing video stream", "Palettizing frames", frames, true);
		...
		try {
			for(uint32 frame=0; frame<frames; ++frame) {
				...
				for(int y=0; y<h; ++y) {
					...
					for(int x=0; x<w; ++x) {
						const uint8 b = (((src[0] * 1286)>>8) + dr[x&15]) >> 8;
						const uint8 g = (((src[1] * 1543)>>8) + dg[x&15]) >> 8;
						const uint8 r = (((src[2] * 1286)>>8) + db[x&15]) >> 8;
						src += 4;
