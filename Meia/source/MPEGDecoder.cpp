...
void VDMPEGDecoder::SetIDCTs(const VDMPEGIDCTSet *pIDCTs) {
	...
	mpBlockDecoder = pIDCTs->pPrescaler ? DecodeBlockPrescaled : DecodeBlockNonPrescaled;
	mbQuantizersDirty = true;
}
...
int VDMPEGDecoder::DecodeFrame(const void *_src, long len, long frame, int dst, int fwd, int rev) {
	...
	switch(type) {
	case 1:		// I-frame
		mpSliceDecoder = DecodeSlice_I;
		break;
	case 2:		// P-frame
		mpSliceDecoder = DecodeSlice_P;
		break;
	case 3:		// B-frame
		mpSliceDecoder = DecodeSlice_B;
