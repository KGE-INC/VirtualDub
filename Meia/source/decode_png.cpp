...
PNGDecodeError VDImageDecoderPNG::Decode(const void *src0, uint32 size) {
	...
	while(src < src_end) {
		...
		unsigned long length = PNGDecodeNetwork32(src);

		if (src_end-src < length+12)
			return kPNGDecodeTruncatedChunk;

		unsigned long crc = PNGDecodeNetwork32(src + length + 8);

		unsigned long type = PNGDecodeNetwork32(src + 4);
