...
static void LogoUpdateOpacityText(HWND hDlg, LogoFilterData *mfd) {
	char buf[32];
	sprintf(buf, "%d%%", (int)(mfd->opacity * (100 / 65536.0) + 0.5));
        ...
        case WM_INITDIALOG:
			{
				...
				SendDlgItemMessage(hDlg, IDC_OPACITY, TBM_SETPOS, TRUE, (int)(mfd->opacity * (100 / 65536.0) + 0.5));
				LogoUpdateOpacityText(hDlg, mfd);
...
static void logo_string2(const FilterActivation *fa, const FilterFunctions *ff, char *buf, int buflen) {
	...
	if (mfd->bEnableAlphaBlending && mfd->bEnableSecondaryAlpha)
		_snprintf(buf, buflen, " (logo:\"%s\", alpha:\"%s\")", SplitPathName(mfd->szLogoPath), SplitPathName(mfd->szAlphaPath));
	else
		_snprintf(buf, buflen, " (logo:\"%s\", alpha:%s)", SplitPathName(mfd->szLogoPath), mfd->bEnableAlphaBlending ? "on" : "off");
}
