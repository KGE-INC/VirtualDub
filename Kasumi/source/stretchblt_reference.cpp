...
namespace {
	void VDSetupNearestSamplingParameters(sint64& u64, sint64 dudx, sint32 dx, sint32 du, sint32& xprecopy, sint32& xprepos, sint32& xmain, sint32& xpostcopy, sint32& xpostpos) {
		...
		if (!tdudx) {
			...
			if (ulo < 0) {
				if (uhi < 0)
					...
					xprecopy = (-ulo-1) / tdudx + 1;

				VDASSERT(xprecopy <= 0 || (uint64)ulo >= ulimit);
				VDASSERT(xprecopy <= 0 || (uint64)(ulo + tdudx * (xprecopy-1)) >= ulimit);
			}
			...
			if (uhi >= ulimit) {
				if (ulo >= ulimit)
					...
					xpostcopy = (uhi - ulimit) / tdudx + 1;

				VDASSERT(xpostcopy <= 0 || (uint64)uhi >= ulimit);
				VDASSERT(xpostcopy <= 0 || (uint64)(uhi - tdudx * (xpostcopy - 1)) >= ulimit);
			}
		...
		VDASSERT(xprecopy <= 0 || (uint64)u64 >= ulimit);
		VDASSERT(xprecopy <= 0 || (uint64)(u64 + dudx * (xprecopy-1)) >= ulimit);
		VDASSERT(xmain <= 0 || (uint64)(u64 + dudx * xprecopy) < ulimit);
		VDASSERT(xmain <= 0 || (uint64)(u64 + dudx * (xprecopy+xmain-1)) < ulimit);
		VDASSERT(xpostcopy <= 0 || (uint64)(u64 + dudx * (xprecopy + xmain)) >= ulimit);
		VDASSERT(xpostcopy <= 0 || (uint64)(u64 + dudx * (xprecopy + xmain + xpostcopy - 1)) >= ulimit);
...
bool VDPixmapStretchBltBilinear_reference(const VDPixmap& dst, sint32 x1, sint32 y1, sint32 x2, sint32 y2, const VDPixmap& src, sint32 u1, sint32 v1, sint32 u2, sint32 v2) {
	...
	if ((uint32)u1 > (uint32)(src.w << 16) || v1 > (uint32)(src.h << 16))
		return false;

	if ((uint32)u2 > (uint32)(src.w << 16) || v2 > (uint32)(src.h << 16))
		return false;
...
#pragma vdpragma_TODO("fixme this is b0rken")
	...
	if (ulo < 0) {
		xprecopy = (1 - ulo) / tdudx + 1;
	}

	const sint64 ulimit = ((sint64)(src.w-1) << 32);

	if (uhi >= ulimit)
		xpostcopy = (uhi - ulimit - 1) / tdudx + 1;
